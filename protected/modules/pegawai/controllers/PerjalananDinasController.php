...
class PerjalananDinasController extends Controller {
    ...
    public function actionCreate() {
        $model = new PerjalananDinas;
        $model->pegawai_id = $this->_pegawai->id;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['PerjalananDinas'])) {
            $model->attributes = $_POST['PerjalananDinas'];
            $model->biaya = 0;
            if ($model->save())
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('PerjalananDinas', array(
                    'criteria' => array(
    ...
    public function actionAdmin() {
        $model = new PerjalananDinas('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['PerjalananDinas']))
            $model->attributes = $_GET['PerjalananDinas'];
    ...
    public function loadModel($id) {
        $model = PerjalananDinas::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'perjalanan-dinas-form') {
